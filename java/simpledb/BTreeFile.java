...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		case BTreePageId.INTERNAL:
			...
			Iterator<BTreeEntry> iter = curPage.iterator();			
			if (iter == null || !iter.hasNext())
				throw new DbException("Bad Iterator<BtreeEnty");
			// if f is null, always get leftchild
			if (f == null)
				return findLeafPage(tid, dirtypages, iter.next().getLeftChild(), perm, f);
			
			BTreeEntry node = iter.next();
			while (true)
			{
				if (node.getKey().compare(Predicate.Op.GREATER_THAN_OR_EQ, f))
				{
					BTreePageId newpid = node.getLeftChild();
					return findLeafPage(tid, dirtypages, newpid, perm, f);
				}
				if (iter.hasNext()) node = iter.next();
				else break;
			}
			return findLeafPage(tid, dirtypages, node.getRightChild(), perm, f);
		case BTreePageId.HEADER:
		case BTreePageId.ROOT_PTR:
		default:
			// this should not happen.  Assuming page is either Leaf or Internal.
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field) 
		...
		BTreeLeafPage curPage = page;
//		System.out.println("Spliting Leaf page:  " + page.getId());
//		System.out.println("cur page parent id" + page.getParentId());
		
		Iterator<Tuple> iter = curPage.reverseIterator();		
		
		// for the split page		
		int tupleToMove = Math.floorDiv(curPage.getNumTuples(), 2);
		//System.out.println("tupleToMove: " + tupleToMove  + ", num of Tuples = " + curPage.getNumTuples());
		BTreeLeafPage pageTwo = (BTreeLeafPage) getEmptyPage(tid, dirtypages, BTreePageId.LEAF );
		
		ArrayList<Tuple> tuples=new ArrayList<Tuple>(0);
		
		for (int i = 0; i < tupleToMove; i++) {
			Tuple t = iter.next();
			tuples.add(t);
		}
		// System.out.println("tuples contains : " + tuples.size()); 
		// move data
		for (int i = tupleToMove - 1 ; i >= 0; i--) {			
			// delete tuple from curPage
			curPage.deleteTuple(tuples.get(i));		
			// insert tuple into newPage
			pageTwo.insertTuple(tuples.get(i));	
		}
		
		//System.out.println("currentPage tuples " + curPage.getNumTuples());
		//System.out.println("New Page tuples " + pageTwo.getNumTuples());
				
		Field newField = tuples.get(tupleToMove-1).getField(keyField);
		//System.out.println("new key value = " + ((IntField) newField).getValue());
		//System.out.println("passed key value = " + ((IntField) field).getValue());
		
		// set siblings
		if (curPage.getRightSiblingId() != null) {		
			BTreeLeafPage pageThree = (BTreeLeafPage) getPage(tid, dirtypages, curPage.getRightSiblingId(), Permissions.READ_WRITE);
			pageThree.setLeftSiblingId(pageTwo.getId());
		}
		pageTwo.setRightSiblingId(curPage.getRightSiblingId());
		curPage.setRightSiblingId(pageTwo.getId());
		pageTwo.setLeftSiblingId(curPage.getId());
		
		// set parent
		BTreeInternalPage parent = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), newField);		
//		System.out.println("Leaf page: getParent parent id" + parent.getId());
		
		// copy up entry to parent internal node
		BTreeEntry newNode = new BTreeEntry(newField, curPage.getId(), pageTwo.getId() );
//		System.out.println("SplitLeafPage: newNode Entry " + newNode.toString());		
		
		try {
			parent.insertEntry(newNode);
		...
		{
			e.printStackTrace();
			throw new DbException("Btree Entry for new node failed to insert into parent node");
		}
		

		if (newField.compare(Op.LESS_THAN_OR_EQ, field))
			return pageTwo;
		else
			return curPage;
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		BTreeInternalPage curPage = page;
//		System.out.println("Spliting Internal page:  " + curPage.getId());
//		System.out.println("cur page parent id" + curPage.getParentId());
		
		Iterator<BTreeEntry> iter = curPage.reverseIterator();		
				
		// for the split page		
		int entryToMove = Math.floorDiv(curPage.getNumEntries(),2);	
		BTreeInternalPage pageTwo = (BTreeInternalPage) getEmptyPage(tid, dirtypages, BTreePageId.INTERNAL );
		//System.out.println("entryToMove: " + entryToMove  + ", num of entries in cur page = " + curPage.getNumEntries());
		
		// collect entries
		ArrayList<BTreeEntry> entries=new ArrayList<BTreeEntry>(0);
		
		for (int i = 0; i < entryToMove; i++) {
			BTreeEntry entry = iter.next();
			entries.add(entry);
		}
		//System.out.println("entries array contains : " + entries.size()); 
		
		// move data
		for (int i = entryToMove - 1 ; i >= 0; i--) {
			BTreeEntry e = entries.get(i);
		
			// delete entry from curPage
			curPage.deleteKeyAndRightChild(e);
						
			// insert entry into newPage
			pageTwo.insertEntry(e);	
			pageTwo.updateEntry(e);		
			}
		
		// get the middle entry to push up
		BTreeEntry curPageLastEntry = iter.next();
		
		curPage.deleteKeyAndRightChild(curPageLastEntry);
			
		//System.out.println("currentPage entries " + curPage.getNumEntries());
		//System.out.println("New Page entries " + pageTwo.getNumEntries());		
			
		//System.out.println("new key value = " + ((IntField) newField).getValue());
		//System.out.println("passed key value = " + ((IntField) field).getValue());

		// update all children's parent pointer in curpage and pageTwo
		updateParentPointers(tid, dirtypages, curPage);
		updateParentPointers(tid, dirtypages, pageTwo);
		
		// push up entry to parent internal node
		Field newField = curPageLastEntry.getKey();	
		BTreeEntry newNode = new BTreeEntry(newField, curPage.getId(), pageTwo.getId() );
//		System.out.println(newNode.toString());	
		
		try {
			BTreeInternalPage parent = getParentWithEmptySlots(tid, dirtypages, curPage.getParentId(), newField);
//			System.out.println("Internal page: getParent parent id" + parent.getId());		
			parent.insertEntry(newNode);
			parent.updateEntry(newNode);
			updateParentPointers(tid, dirtypages, parent);
		}
		catch (DbException e)
		{
			e.printStackTrace();
			throw new DbException("Parent pointer update failed");
		}
		if (newField.compare(Op.LESS_THAN_OR_EQ, field))
			return pageTwo;
		else
	 ...
	 * @param keyField2 - the key of the entry which will be inserted. Needed in case the parent must be split
	...
	private BTreeInternalPage getParentWithEmptySlots(TransactionId tid, HashMap<PageId, Page> dirtypages, 
			BTreePageId parentId, Field keyField2) throws DbException, IOException, TransactionAbortedException {
		...
		if(parent.getNumEmptySlots() == 0) {
			parent = splitInternalPage(tid, dirtypages, parent, keyField2);
		}
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		int minTuple = sibling.getMaxTuples()/2;
		int curPageNumTuple = page.getNumTuples();
		int siblingNumTuple = sibling.getNumTuples();
		int evenNum = (curPageNumTuple + siblingNumTuple)/2;
		BTreeEntry newNode = null;
		
		if (siblingNumTuple < minTuple || evenNum < minTuple)
			throw new DbException("Sibling has less than min tuples; cannot steal from sibling");
		
		int numToSteal = siblingNumTuple - evenNum;
		
		if (isRightSibling) {
			// use the iterator to collect tuples in tuples array
			Iterator<Tuple>	siblingIter = sibling.iterator();
			ArrayList<Tuple> tuples=new ArrayList<Tuple>(0);
		
			for (int i = 0; i < numToSteal; i++) {
				Tuple t = siblingIter.next();
				tuples.add(t);
			}
		// System.out.println("tuples contains : " + tuples.size()); 
			// move data
			for (int i = 0; i < numToSteal ; i++) {			
				// delete tuple from curPage
				sibling.deleteTuple(tuples.get(i));		
			// insert tuple into newPage
				page.insertTuple(tuples.get(i));
			// old field is in tuple[0]; now move to current page
			// Field oldField = tuples.get(0).getField(keyField);
			Field newField = siblingIter.next().getField(keyField);	
			//BTreeEntry oldNode = new BTreeEntry(oldField, page.getId(), sibling.getId());
			newNode = new BTreeEntry(newField, page.getId(), sibling.getId() );
			}	
		}
		else { // stealing from left sibling
			// use the reverseIterator to collect tuples in tuples array
			Iterator<Tuple>	siblingIter = sibling.reverseIterator();
			ArrayList<Tuple> tuples=new ArrayList<Tuple>(0);
			Iterator<Tuple>  pageIter = page.iterator();
			
			// save the tuple field from current page 
			Field oldField = pageIter.next().getField(keyField);
			...
			for (int i = numToSteal - 1; i >=0 ; i--) {			
				// delete tuple from sibling
				sibling.deleteTuple(tuples.get(i));		
				// insert tuple into current page
				page.insertTuple(tuples.get(i));
			}	
				// the new field is in tuple[numToSteal-1]
			Field newField = tuples.get(numToSteal -1).getField(keyField);
			//BTreeEntry oldNode = new BTreeEntry(oldField, sibling.getId(), page.getId());
			newNode = new BTreeEntry(newField, sibling.getId(), page.getId());
		}
		
		// remove the oldNode from parent
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		// some code goes here
		int minEntries = leftSibling.getMaxEntries()/2;
		int curPageNumEntry = page.getNumEntries();
		int siblingNumEntry = leftSibling.getNumEntries();
		int evenNum = (curPageNumEntry + siblingNumEntry)/2;
		BTreeEntry newNode = null;
		
		//System.out.println("stealFromLeftInternalPage: minEntries " + minEntries);
		//System.out.println("Page count " + curPageNumEntry + "Sibling count: " + siblingNumEntry);
		//System.out.println("redistribute target:  " + evenNum);
		
		// decide on how many entries to steal
		if (evenNum < minEntries || siblingNumEntry < minEntries) {
			mergeInternalPages(tid, dirtypages, leftSibling, page, parent, parentEntry);
			return;
		}
		int entriesToSteal = (siblingNumEntry - curPageNumEntry)/2;
		
		Iterator<BTreeEntry> sibIter = leftSibling.reverseIterator();

		BTreeEntry newEntry = new BTreeEntry(parentEntry.getKey(), null,  page.iterator().next().getLeftChild());

		for (int i = 0; i < entriesToSteal; i++) {
			BTreeEntry entryToLeft = sibIter.next();
			newEntry.setLeftChild(entryToLeft.getRightChild());
			page.insertEntry(newEntry);	
			newEntry = new BTreeEntry(entryToLeft.getKey(), null, entryToLeft.getRightChild());
			leftSibling.deleteKeyAndRightChild(entryToLeft);
		}
		
		//System.out.println("LeftPage entries " +leftSibling.getNumEntries());
		//System.out.println("Page entries " + page.getNumEntries());		

		try {
		// the new entry to push up to parent
		// update all children's parent pointer in page and leftsibling
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		int minEntries = rightSibling.getMaxEntries()/2;
		int curPageNumEntry = page.getNumEntries();
		int siblingNumEntry = rightSibling.getNumEntries();
		int evenNum = (curPageNumEntry + siblingNumEntry)/2;
		//BTreeEntry newNode = null;
		
		//System.out.println("stealFromRightInternalPage: minEntries " + minEntries);
		//System.out.println("Page count " + curPageNumEntry + "Sibling count: " + siblingNumEntry);
		//System.out.println("redistribute target:  " + evenNum);
		
		// decide on how many entries to steal
		if (evenNum < minEntries || siblingNumEntry < minEntries) {
			mergeInternalPages(tid, dirtypages, page, rightSibling, parent, parentEntry);
			return;
		}
		int entriesToSteal = (rightSibling.getNumEntries() - page.getNumEntries())/2;
		Iterator<BTreeEntry> sibIter = rightSibling.iterator();

		// the first goes into stealing array
		// newEntry contains left child from page's last entry's right child
		//                     right child from right sib's first entry's left child
		BTreeEntry newEntry = new BTreeEntry(parentEntry.getKey(), page.reverseIterator().next().getRightChild(), null);

		for (int i = 0; i < entriesToSteal; i++) {
			BTreeEntry entryToRight = sibIter.next();

			newEntry.setRightChild(entryToRight.getLeftChild());
			page.insertEntry(newEntry);
			newEntry = new BTreeEntry(entryToRight.getKey(), entryToRight.getLeftChild(), null);
			rightSibling.deleteKeyAndLeftChild(entryToRight);			
			}
		//System.out.println("currentPage entries " + page.getNumEntries());
		//System.out.println("Right Sibling entries " + rightSibling.getNumEntries());		

		try {					
		// correct entry in parent internal node
		parentEntry.setKey(newEntry.getKey());
		parent.updateEntry(parentEntry);	
		
		// update all children's parent pointer in page and rightsibling
		updateParentPointers(tid, dirtypages, page);
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		if (rightPage.getRightSiblingId() != null) {
			BTreeLeafPage neighbor = (BTreeLeafPage) getPage(tid, dirtypages, rightPage.getRightSiblingId(), Permissions.READ_WRITE);//		if (neighbor != null) {		
		...
		// update BTreeEntry in parent
		deleteParentEntry(tid, dirtypages, leftPage, parent, parentEntry);
		} 
		catch (DbException e)
		{
			e.printStackTrace();
			throw new DbException("mergeLeafPage: db exception in update sibling/parent");
		}
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		// the first entry pulled down from parent entry; update children before insert the entry
		BTreeEntry parentNode = new BTreeEntry(parentEntry.getKey(), leftPage.reverseIterator().next().getRightChild(), rightPage.iterator().next().getLeftChild());
		leftPage.insertEntry(parentNode);
		...
		while  (iter.hasNext()) {	
			...
			// insert tuple into leftPage
			leftPage.insertEntry(ent);	
		}
		...
		{
			e.printStackTrace();
			throw new DbException("mergeInternalPage: db exception in update sibling/parent");
		}
